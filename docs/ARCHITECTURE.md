...
- `chaining.py` — rule-based multi-step attack paths across vuln classes (e.g. kerberoast → delegation → domain compromise), boosting chained findings.
- `cvss.py` / `vulnclasses.py` / `classify.py` — CVSS v3.1 scoring, the vuln-class taxonomy (CWE + OWASP + typical vector), and signal→class inference that grounds/adapts each decision.
